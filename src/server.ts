...
export function createApp(todoStore: TodoStore, projectStore?: ProjectStore): http.Server {
  return http.createServer((req, res) => {
    ...
    const projectTodosSegments = pathname.match(/^\/projects\/([^/]+)\/todos$/);
    if (req.method === "GET" && projectTodosSegments && projectStore) {
      const id = decodeURIComponent(projectTodosSegments[1]);
      ...
      const todos = todoStore.listByProject(id);
      res.writeHead(200, { "Content-Type": "application/json" });
